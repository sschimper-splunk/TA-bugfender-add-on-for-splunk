...
def get_bugfender_app_devices(helper):
    ...
    params = {
        "date_range_start": get_latest_timestamp(helper),
        "date_range_end": query_start_timestamp.strftime("%Y-%m-%dT%H:%M:%SZ"),
        "format": "json",
        "order": "seen",
        "page_size": "100"
    }
...
def get_bugfender_app_issues(helper):
    ...
    unix_timestamp = datetime(1970, 1, 1, tzinfo=pytz.utc).strftime("%Y-%m-%dT%H:%M:%SZ")

    params = {
        "date_range_start": get_latest_timestamp(helper) if get_latest_timestamp(helper) else unix_timestamp,
    ...
    # DEEEBUG
    # params["date_range_start"] = "2023-08-07T16:24:00Z"
...
def get_bugfender_app_logs(helper):
    ...
    # DEEEEBUG
    params["date_range_start"] = "2023-07-09T00:00:00Z"
    ...
    helper.log_debug(f"[>] get_bugfender_app_logs() - last URL is {url}")
