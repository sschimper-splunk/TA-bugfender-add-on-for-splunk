...
def validate_input(helper, definition):
    ...
    bf.validate_rfc3339(helper.get_arg("start_time"))
...
def collect_events(helper, ew):
    ...
    last_timestamp_checkpoint_key = f"{helper.get_input_stanza_names()}_bugfender_app_logs_last_timestamp"
    ...
    response = bf.get_bugfender_data(helper, api_endpoint, start_date)

    # convert response from ndsjon string to Python list of Python doctionaries
    formatted_data = response.json(cls=ndjson.Decoder)

    helper.log_debug(f"Starting to write data queried from enpoint {api_endpoint} to Splunk.")
    ...
    # helper.log_debug(f"Data queried from endpoint {api_endpoint} successfully written to Splunk. {duplicate_count} duplicate events were encountered and skipped.")
    helper.log_debug(f"Data queried from endpoint {api_endpoint} successfully written to Splunk. {event_count} in total were indexed.")
